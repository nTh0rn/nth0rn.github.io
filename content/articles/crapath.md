---
external: false
title: "CraPath, The Worst Pathfinding Algorithm (C++)"
description: "A pathfinding algorithm this is intentionally obtuse"
tags: ["C++", "Joke code", "Pathfinding", "Maze generation", "CraPath", "Crappy pathfinder"]
date: 2024-08-13
draft: true
---
...
# 1. Intro
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;This article explores the development of CraPath, a joke pathfinding algorithm that is intentionally (for the most part) a bit obtuse. This is a pathfinding algorithm I first conceptualized when I was 13, before I understood A* or any other algorithms that are computational effecient.

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;This algorithm is by all means awful, but it was the best that I could comprehend when I was 13 with the tools I had (GameMaker Studio 1.4). At the time, I had never successfully implemented CraPath, but I recently decided that I should fulfil 13-year-old-me's dreams.

## 1.1 Demo / Source Code
[Source code on GitHub](https://github.com/nTh0rn/batch-raycaster)

## 2. CraPath Pre-requisites
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;It's important to understand exactly what environment CraPath can function in, and how traversing actually functions. CraPath must be in an environment where it is possible to cast rays and check for obstructions between two points on the map. In the case of this article, this is accomplished by having a grid where cells can either be empty or contain walls. Whether or not any two cells can see eachother without any obstructions is determined using [DDA raycasting](https://lodev.org/cgtutor/raycasting.html#The_Basic_Idea_), which functions extremely well in a grid environment.

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Traversal is accomplished using a single 2D vector that contains the pairs of x and y coordinates needed to be traversed linearly to reach the goal. The gaps between these nodes can also be added to the final path vector to better visualize traversal.

## 2.1 CraPath Algorithm

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;There are three main steps to CraPath. First, a randomly generated path is found. This path consists only of nodes that have sightline of eachother, meaning there can be large jumps across the map.
...
APPEND goal to path[]
```
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Now that a path has been found, we fill in all the cells between the nodes. This allows the later optimization to have more options for skipping unnecessary nodes.
...
END FOR
```
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Finally, we look iteratively at the front and back of the path and find nodes that can be skipped entirely. We erase these nodes from the path until no more optimizations can be made.

```
FOR each node at the start of path[] moving forwards, i
    FOR each node at the end of path[] moving backwards, j
        IF i is right next to j THEN
            BREAK
        END IF
...
{% octant_pic url="/images/batch_raycaster/octants_diagram.png"/%}
